...
use serde_json::{Number, Result as JsonResult, Value};
        ...
        match stream.next().await {
            Some(response) => {
                ...
                if let Some(found_entry) = find_log_entry("init_pc_amount", log_entries) {
                    ...
                    let Ok(tx) = solana_client.get_transaction_with_config(&signature, config)
                    else {
                        continue;
                    };

                    let inner_instructions: Vec<UiInnerInstructions> = tx
                        .transaction
                        .meta
                        .as_ref()
                    ...
                    let log_messages: Vec<String> = tx
                        ...
                        .unwrap();
                    let pre_token_balances: Vec<UiTransactionTokenBalance> = tx
                        .transaction
                        .meta
                        .as_ref()
                    ...
                    let info: PoolInfo = parse_pool_info_from_lp_transaction(
                        tx,
                        &inner_instructions,
                        &raydium_program_id,
                        &wrapped_sol,
                    ...
                    .expect("PoolInfo_error");
                    let market_info = fetch_market_info(&solana_client, &info.market_id).await;
                    let keyz: LiquidityPoolKeys =
                        create_pool_key(&info, &market_info.expect("Market_info_no_errors"));

                    let duration = start.elapsed();
                    println!("Whole process took {}", duration.as_millis());
                    let open_time = DateTime::from_timestamp(info.open_time as i64, 0)
                        .expect("invalid timestamp");
                    let elapsed = Utc::now().time() - open_time.to_utc().time();
                    println!("Elapsed since pool creation {}", elapsed.num_seconds());
                    println!("Whole process took {}", duration.as_millis());
...
async fn perform_swap(
    ...
    if key_z.base_mint.to_string() != "So11111111111111111111111111111111111111112".to_string()
        && key_z.quote_mint.to_string() == "So11111111111111111111111111111111111111112".to_string()
    {
        let token_account_in = get_or_create_associated_token_account(
            client,
            key_payer,
            wallet_address,
            &key_z.quote_mint,
        ...
        let token_account_out = get_or_create_associated_token_account(
            client,
            key_payer,
            wallet_address,
            &key_z.base_mint,
        ...
        let params: LiquiditySwapFixedInInstructionParamsV4 =
        ...
        // };
        let mut transaction =
            Transaction::new_with_payer(&[the_swap_instruction], Some(&wallet_address));
        ...
        loop {
            ...
            match result {
                Ok(signature) => {
                    println!(
                        "Transaction succeeded with signature: https://solscan.io/tx/{}",
                        signature
                    );
                    break;
                }
...
async fn fetch_market_info(
    client: &RpcClient,
    market_id: &Pubkey,
) -> Result<MarketStateLayoutV3, Box<dyn std::error::Error>> {
    ...
    if data.is_empty() {
        println!(
            "{:?}",
            format!(
                "Failed to fetch market info for market id {}",
                market_id.to_string()
            )
        );
    }
            ...
            match message {
                UiMessage::Parsed(ins) => {
                    ...
                    let init_instructions =
                        find_instruction_by_program_id(&instructions, &raydium_program_id).unwrap();
                        ...
                        UiInstruction::Parsed(inside) => {
                            match inside {
                                UiParsedInstruction::PartiallyDecoded(parsed) => {
                                    ...
                                    let lp_decimals: u8 = get_decimals(&lp_init_mint_instruction)
                                        .expect("wrong_lp_decimals");
                                    let lp_ac: String = get_info_ac(&lp_mint_mint_instruction)
                                        .expect("lp_ac error");
                                    let open_time: u64 =
                                        extract_open_time(&lp_initialization_log_entry_info)
                                    ...
                                    let base_reserves: String =
                                        get_info_Amount(&base_transfer_instruction)
                                            .expect("err_base_reserves");
                                    let quote_reserves: String =
                                        get_info_Amount(&quote_transfer_instruction)
                                            .expect("err_quote_reserves");
                                    let lp_reserves: String =
                                        get_info_Amount(&lp_mint_mint_instruction)
                                            .expect("reserves_err");
                ...
                match &parsed_instruction {
                    UiParsedInstruction::Parsed(instruct) => {
                        ...
                        if extract_type_field(data).unwrap() == "transfer".to_string()
                            && extract_destination_from_info(data).unwrap().to_string()
                                == destination_account.to_string()
                            && (program_id.is_none()
                                || program_ide.to_string() == program_id.unwrap().to_string())
                        {
        ...
        for instruction in &inner.instructions {
            if let UiInstruction::Parsed(parsed_instruction) = instruction {
                match &parsed_instruction {
                    UiParsedInstruction::Parsed(instruct) => {
                        let data: &Value = &instruct.parsed;
                        if extract_type_field(data).unwrap() == "initializeMint".to_string()
                            && extract_mint_from_info(data).unwrap() == mint_address.to_string()
                        {
        ...
        for instruction in &inner.instructions {
            if let UiInstruction::Parsed(parsed_instruction) = instruction {
                match &parsed_instruction {
                    UiParsedInstruction::Parsed(instruct) => {
                        let data: &Value = &instruct.parsed;
                        if extract_type_field(data).unwrap() == "mintTo".to_string()
                            && extract_mint_from_info(data).unwrap() == mint_address.to_string()
                        {
